...
impl OuiDataBuilder {
    ...
    fn build(&mut self, entries: Vec<OuiEntry>) {
        ...
        for (prefix3, entries_list) in entries_28 {
            ...
            if !entries_24.contains_key(&prefix3) {
                self.subtable_only_prefixes.push((prefix3, SubtableRef::Oui28(subtable_idx), entry_count));
            }
        }
        ...
        for (prefix3, entries_list) in entries_32 {
            ...
            // 只有当 entries_24 中不存在该前缀时，才记录为 subtable_only
            if !entries_24.contains_key(&prefix3) {
                self.subtable_only_prefixes.push((prefix3, SubtableRef::Oui32(subtable_idx), entry_count));
            }
        }
        ...
        for (prefix3, entries_list) in entries_36 {
            ...
            // 只有当 entries_24 中不存在该前缀时，才记录为 subtable_only
            if !entries_24.contains_key(&prefix3) {
                self.subtable_only_prefixes.push((prefix3, SubtableRef::Oui36(subtable_idx), entry_count));
            }
        }
    ...
    fn generate_code(&self, output_path: &Path) {
        ...
        code.push_str(&format!("// - Total length of names: {} bytes\n", unique_names_total_length));
        code.push_str(&format!("// - OUI-24 entries: {}\n", oui24_prefixes.len()));
        code.push_str(&format!("// - OUI-28 tables: {}\n", self.oui28_tables.len()));
        code.push_str(&format!("// - OUI-32 tables: {}\n", self.oui32_tables.len()));
        code.push_str(&format!("// - OUI-36 tables: {}\n", self.oui36_tables.len()));
        code.push_str(&format!("// - Subtable-only prefixes: {}\n", self.subtable_only_prefixes.len()));
        ...
        if !self.subtable_only_prefixes.is_empty() {
            ...
            code.push_str("// These prefixes have only subtables (28/32/36-bit) without a 24-bit OUI entry\n");
            ...
            for (prefix, subtable_type, entry_count) in &self.subtable_only_prefixes {
                ...
                code.push_str(&format!("// - {:02X}{:02X}{:02X} ({}, {} entries)\n", 
        ...
        for (prefix, subtable_type, entry_count) in &self.subtable_only_prefixes {
            ...
            println!("cargo:warning=  {:02X}{:02X}{:02X} ({}, {} entries)", 
                prefix[0], prefix[1], prefix[2], type_str, entry_count);
        }
        ...
        // 更新文件大小统计（替换占位符）
        let code = code.replace("// - Generated file size: 0 bytes", 
        ...
        println!("cargo:warning=Generated {} with {} bytes", output_path.display(), code.len());
        println!("cargo:warning=Statistics:");
        println!("cargo:warning=  - Unique names: {}", self.names.len());
        println!("cargo:warning=  - Total length of names: {}", unique_names_total_length);
        println!("cargo:warning=  - OUI-24 entries: {}", oui24_prefixes.len());
        println!("cargo:warning=  - OUI-28 tables: {}", self.oui28_tables.len());
        println!("cargo:warning=  - OUI-32 tables: {}", self.oui32_tables.len());
        println!("cargo:warning=  - OUI-36 tables: {}", self.oui36_tables.len());
        println!("cargo:warning=  - Subtable-only prefixes: {}", self.subtable_only_prefixes.len());
    }
}
